<?php
class ControllerWebExtensionAccountPurpletreeMultivendorSellerregister extends Controller
{
	private $error = array();
	...
	public function index()
	{
		...
		if (!$this->customer->validateSeller($livecheck)) {
			...
			$this->response->redirect($this->url->link('web/account/register', '', true));
		}
		$data['loggedcus'] = '';
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if ($customer_group_info['approval']) {
				if ($this->config->get('module_purpletree_multivendor_seller_approval') == 1) {
					...
					$login_url = $this->url->link('web/extension/account/purpletree_multivendor/sellerlogin', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('web/extension/account/purpletree_multivendor/sellerregister', '', true)
		);
		$this->load->model('extension/purpletree_multivendor/vendor');
		...
		$data['action'] = $this->url->link('web/extension/account/purpletree_multivendor/sellerregister', '', true);
		$data['sellerlogin'] = $this->url->link('web/extension/account/purpletree_multivendor/sellerlogin', '', true);
		...
		if ($this->config->get('config_account_id')) {
			...
			if ($information_info) {
				$data['text_agree'] = sprintf($this->language->get('text_agree'), $this->url->link('web/information/information/agree', 'information_id=' . $this->config->get('config_account_id'), true), $information_info['title'], $information_info['title']);
			} else {
				$data['text_agree'] = '';
			}
		} else {
	...
	public function subscribePlan($customer_id)
	{
		...
		if ($plan_id) {
			...
			$reciver = $this->config->get('config_email');
			$this->model_extension_purpletree_multivendor_vendor->ptsSendMail($reciver, $email_subject, $email_message);
			//$this->response->redirect($this->url->link('web/extension/account/purpletree_multivendor/subscriptionplan/invoice', '' . 'invoice_id=' . $invoice_id .'&old_invoice_id='.$old_invoice_id. $url, true));
			//}
		}
		//$this->getplan();
	}
